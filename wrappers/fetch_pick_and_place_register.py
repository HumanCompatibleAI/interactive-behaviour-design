    ...
    for s1, repeat in repeat_ns:
        for s2, binary_gripper in binary_grippers:
            for s3, include_grip_obs in include_grip_obss:
                for s4, full_obs in full_obss:
                    for s5, n_initial_block_positions in n_initial_block_positionss:
                        for s6, fixed_goal in fixed_goals:
                            for s7, early_termination in early_terminations:
                                for s8, slow_gripper in slow_grippers:
                                    for s9, vanilla_rl in vanilla_rls:
                                        for s10, mode in modes:
                                            for s11, gripper_bonus in gripper_bonuses:
                                                env_id = f'FetchPickAndPlace-{s1}-{s2}-{s3}-{s4}-{s5}-{s6}-{s7}-{s8}-{s9}-{s10}-{s11}-v0'
